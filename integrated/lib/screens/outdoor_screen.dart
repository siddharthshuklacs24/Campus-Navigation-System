...
class _OutdoorScreenState extends State<OutdoorScreen> {
  ...
  Future<void> calculateRoute() async {
    ...
    try {
      ...
      String startNode;
      if (fromText == "CURRENT LOCATION" || fromText == "MY LOCATION") {
        startNode = _findNearestEntrance(allNodesCoordinates.keys.toList(), startRef);
      } else if (targetPOIs.containsKey(fromText)) {
        startNode = _findNearestEntrance(targetPOIs[fromText]!, startRef);
      } else {
        startNode = fromText;
      }
      ...
      if (routePoints.isNotEmpty) {
        mapController.fitCamera(CameraFit.bounds(
          ...
          padding:
              const EdgeInsets.only(top: 240, left: 50, right: 50, bottom: 50),
        ));
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // ✅ APPBAR REMOVED: Reverted to the clean map UI. 
      // Indoor navigation is handled entirely through tapping the POI pins now.
      
      floatingActionButton: Column(
        mainAxisSize: MainAxisSize.min, // Stops invisible gesture stealing shield
        ...
        children: [
          ...
          FloatingActionButton(
            ...
            child: Icon(Icons.bug_report,
                color: isDebugMode ? Colors.white : Colors.grey),
          ),
          const SizedBox(height: 10),
          FloatingActionButton(
            ...
            onPressed: () =>
                setState(() => showGoogleLabels = !showGoogleLabels),
            child: Icon(showGoogleLabels ? Icons.label : Icons.label_off,
                color: showGoogleLabels ? Colors.white : Colors.blueAccent),
          ),
          const SizedBox(height: 10),
          FloatingActionButton(
            ...
            child:
                const Icon(Icons.layers_rounded, color: Colors.blueAccent),
          ),
        ],
      ),
      body: Stack(
          ...
          FlutterMap(
            ...
            options: MapOptions(
              ...
              interactionOptions: const InteractionOptions(
                flags: InteractiveFlag.all, // Explicitly accepts all gestures
              ),
            ),
            children: [
              TileLayer(
                ...
                markers: [
                  ...
                  if (!isDebugMode && allNodesCoordinates.isNotEmpty)
                    ...targetPOIs.keys.map((poiName) {
                      ...
                      if (pos == null) {
                        return Marker(
                            point: campusCenter, child: const SizedBox());
                      }
                        ...
                        child: GestureDetector(
                          onTap: () {
                            if (poiName == "MESS BLOCK") {
                              ...
                              setState(() =>
                                  toSearchController.text = poiName);
                            }
                          },
                          child: Column(
                            children: [
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(
                      isLoading ? "Finding Path..." : "Get Campus Directions"),
  ...
  Widget _buildSearchField(SearchController controller, String hint) {
    return SearchAnchor(
      ...
      suggestionsBuilder: (context, controller) {
        ...
        final matches = targetPOIs.keys
            .where((poiName) => poiName.contains(query))
            .toList();

        return [
          if (hint == "Starting Point")
            ListTile(
              leading: const Icon(Icons.my_location, size: 20),
              title: const Text("Current Location"),
              onTap: () {
                setState(() {
                  controller.closeView("CURRENT LOCATION");
                });
              },
            ),
          ...matches.map((poiName) => ListTile(
